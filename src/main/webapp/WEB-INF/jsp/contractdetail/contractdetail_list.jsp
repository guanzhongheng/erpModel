...
<body style="background-color: rgba(255,255,255,0);">
      ...
      <div class="card" style="margin-top:5px;">
         <div class="card-body">
			...
			<!-- 检索  -->
			<form action="contractdetail/list" method="post" name="Form" id="Form">
					...
					<c:choose>
						<c:when test="${not empty varList}">
							<c:forEach items="${varList}" var="var" varStatus="vs">
								<tr>
									<td>
										<input type="checkbox" style="margin-top: 2px;" name='ids' value="${var.CONTRACTDETAIL_ID}">
									</td>
									...
									<td>${var.PROD_TYPE}</td>
									<td>${var.QUANTITY}</td>
									<td>${var.PROD_MANAGER}</td>
									<td>${var.OVERAGE}</td>
									<td>${var.OQA}</td>
									<td>${var.OQA_USER}</td>
									<td>${var.CREATE_TIME}</td>
									<td>
										<a title="修改" onclick="edit('${var.CONTRACTDETAIL_ID}');"><i class="zmdi zmdi-edit zmdi-hc-fw"></i></a>
	                 					<a title="删除" onclick="del('${var.CONTRACTDETAIL_ID}');"><i class="zmdi zmdi-close zmdi-hc-fw"></i></a>
									</td>
								</tr>
				...
				<table style="width:100%;margin-top:15px;">
					<tr>
						<td style="vertical-align:top;">
							<a class="btn btn-light btn--icon-text" onclick="add();">新增</a>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>contractdetail/goAdd?_ID=${pd._ID}';
			 diag.Width = 800;
			 diag.Height = 600;
		...
		function del(Id){
			swal({
            	...
            	$.ajax({
        			type: "POST",
        			url: '<%=basePath%>contractdetail/delete',
        	    	data: {CONTRACTDETAIL_ID:Id,tm:new Date().getTime()},
        			dataType:'json',
        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>contractdetail/goEdit?CONTRACTDETAIL_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
			...
			swal({
    			...
    			if(str==''){
    				...
    				if(msg == '确定要删除选中的数据吗?'){
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>contractdetail/deleteAll?tm='+new Date().getTime(),
    				    	data: {DATA_IDS:str},
    						dataType:'json',
    						cache: false,
    						success: function(data){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>contractdetail/excel';
		}
