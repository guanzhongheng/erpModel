...
<body style="background-color: rgba(255,255,255,0);">
      ...
      <div class="card" style="margin-top:5px;">
         <div class="card-body">
			...
			<!-- 检索  -->
			<form action="prorecord/list" method="post" name="Form" id="Form">
				...
				<c:choose>
					<c:when test="${not empty varList}">
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								<td>
									<input type="checkbox" style="margin-top: 2px;" name='ids' value="${var.PRORECORD_ID}">
								</td>
								...
								<td>
									<shiro:hasPermission name="prorecord:edit"><a title="修改" onclick="edit('${var.PRORECORD_ID}');"><i class="zmdi zmdi-edit zmdi-hc-fw"></i></a></shiro:hasPermission>
                 					<shiro:hasPermission name="prorecord:del"><a title="删除" onclick="del('${var.PRORECORD_ID}');"><i class="zmdi zmdi-close zmdi-hc-fw"></i></a></shiro:hasPermission>
								</td>
							</tr>
			...
			<table style="width:100%;margin-top:15px;">
				<tr>
					<td style="vertical-align:top;">
						<shiro:hasPermission name="prorecord:add"><a class="btn btn-light btn--icon-text" onclick="add();">新增</a></shiro:hasPermission>
						<shiro:hasPermission name="prorecord:del"><a class="btn btn-light btn--icon-text" onclick="makeAll('确定要删除选中的数据吗?');">删除</a></shiro:hasPermission>
					</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>prorecord/goAdd';
			 diag.Width = 600;
			 diag.Height = 500;
		...
		function del(Id){
			swal({
            	...
            	$.ajax({
        			type: "POST",
        			url: '<%=basePath%>prorecord/delete',
        	    	data: {PRORECORD_ID:Id,tm:new Date().getTime()},
        			dataType:'json',
        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>prorecord/goEdit?PRORECORD_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 500;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
			...
			swal({
    			...
    			if(str==''){
    				...
    				if(msg == '确定要删除选中的数据吗?'){
    					$.ajax({
    						type: "POST",
    						url: '<%=basePath%>prorecord/deleteAll?tm='+new Date().getTime(),
    				    	data: {DATA_IDS:str},
    						dataType:'json',
    						cache: false,
    						success: function(data){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>prorecord/excel';
		}
