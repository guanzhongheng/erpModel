...
(function($){
	...
	$.fn.edatagrid.methods = {
        ...
        deleteRow: function(jq, index) {
			...
			opts.finder.getTr(jq[0], index).remove();
			var gd = $.data(jq[0], "datagrid");
			for (var i = index + 1; i < data.rows.length; i++) {
				opts.finder.getTr(jq[0], i, "body", 2).attr("datagrid-row-index", i - 1)
					.attr("id",gd.rowIdPrefix+"-2-"+(i-1));
				var tr1 = opts.finder.getTr(jq[0], i, "body", 1).attr("datagrid-row-index", i - 1)
