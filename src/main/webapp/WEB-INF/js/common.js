...
var TT = TAOTAO = {
    ...
    init : function(data,image){
    	// 初始化图片上传组件
    	this.initPicUpload(data,image);
    },
    // 初始化图片上传组件
    initPicUpload : function(data,image){
        			...
        			if($.trim(imgs[i]).length > 0){
        				_ele.siblings(".pics").find("ul").append("<li><a id='img"+i+"' href='"+imgs[i]+"' target='_blank'>" +
        						"<img src='"+imgs[i]+"' width='80' height='50' /></a> " +
        							"<a id='del"+i+"' href='javascript:removeImg("+i+","+image+");'>" +
        		...
        		KindEditor.editor(TT.kingEditorParams).loadPlugin('multiimage',function(){
        			...
        			editor.plugin.multiImageDialog({
						clickFn : function(urlList) {
							...
							var origin = form.find("[name=attachment]").val();
							if(origin != null){
								var originUrls = origin.split(",");  
								for(var i in originUrls){
									imgArray.push(originUrls[i]);
							...
							form.find("[name=attachment]").val(imgArray.join(","));
							editor.hideDialog();
						}
					});
        		});
